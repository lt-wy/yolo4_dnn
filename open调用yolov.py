...
LABELS = open("train.names").read().strip().split("\n")
np.random.seed(666)
COLORS = np.random.randint(0, 255, size=(len(LABELS), 3), dtype="uint8")
# 导入 YOLO 配置和权重文件并加载网络：
net = cv2.dnn_DetectionModel('2.cfg', '2_best.weights')
...
#调用摄像头
# image=cv2.VideoCapture(0) #cv2.VideoCapture(0)代表调取摄像头资源，其中0代表电脑摄像头，1代表外接摄像头(usb摄像头)
# image.set(3,1040)#宽
# image.set(4,880)#高
# image.set(10,100)#亮度
# while True:
